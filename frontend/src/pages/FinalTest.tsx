...
export default function FinalTest() {
  ...
  const handleStart = useCallback(async () => {
    try {
      ...
      const testResponse = await startPlacementTest({ testType: "Final" });
    ...
    <AnimatePresence mode="wait">
      {currentStep === "intro" && (
        ...
        >
          <PlacementTestIntro 
            onStart={handleStart}
            title="Bài kiểm tra kết thúc học"
            description="Đánh giá tổng thể kết quả học tập sau khi hoàn thành toàn bộ khóa học và so sánh với mục tiêu ban đầu."
          />
        </motion.div>
      )}
