 ...
 * Task 2 Page
 * 
 * Example page showing how to use Task 2 topic hooks
...
import { 
  ...
  useRandomTask2Topic
} from '../hooks/useTask2Topics';
import type { Task2Type } from '../types/task2-topic';
import { Loader2, Sparkles, Shuffle, BookOpen, FileText } from 'lucide-react';
...
export default function Task2Page() {
  ...
  const { 
    ...
    error: academicError 
  } = useTask2Topics({ taskType: 'Academic' });

  // Example 2: Get recommended topics
  const { 
    data: recommendedTopics, 
    loading: recommendedLoading 
  } = useRecommendedTask2Topics({ count: 3, taskType: 'Academic' });

  // Example 3: Get topic detail (when a topic is selected)
  const { 
    data: topicDetail, 
    loading: detailLoading 
  } = useTask2TopicDetail(selectedTopicId);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-indigo-50 dark:from-gray-900 dark:via-purple-900/20 dark:to-indigo-950 py-8 px-4">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            IELTS Writing Task 2 Essay Practice Topics
          </p>
        </div>
              ...
              {academicTopics.map((topic) => (
                <div
                  ...
                  className={`border rounded-lg p-4 hover:shadow-md transition-all cursor-pointer ${
                    selectedTopicId === topic.id ? 'ring-2 ring-purple-500' : ''
                  }`}
                  onClick={() => setSelectedTopicId(topic.id)}
                >
