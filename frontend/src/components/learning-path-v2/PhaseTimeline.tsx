...
import type { Phase, PhaseTask1Topic, PhaseTask2Topic } from '../../types/learningPathTypes';
import { 
  getPhaseStatusColor, 
  getPhaseStatusBg, 
  getPhaseStatusLabel,
...
function PhaseCardItem({ phase, index, isExpanded, onToggle, onStartTopic }: PhaseCardItemProps) {
  ...
  const getTimelineIconBg = () => {
    switch (phase.status) {
      ...
      case 'InProgress': return 'bg-indigo-500 ring-4 ring-indigo-200 dark:ring-indigo-900';
  ...
  return (
    ...
    >
      {/* Timeline node */}
      <div className={`absolute left-4 w-5 h-5 rounded-full ${getTimelineIconBg()} flex items-center justify-center z-10`}>
        ...
        >
          <div className="flex items-center gap-4">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold ${
              ...
              phase.status === 'InProgress' ? 'bg-gradient-to-br from-indigo-500 to-purple-600' :
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${getPhaseStatusBg(phase.status)} ${getPhaseStatusColor(phase.status)}`}>
                    ...
                    {activeTab === 'task1' ? (
                      phase.task1Topics.map((topic) => (
                        <TopicItem 
                          ...
                          topic={topic} 
                          type="task1"
                          onStart={() => onStartTopic(topic.id, 'task1')}
                        />
                      ))
                    ) : (
                      phase.task2Topics.map((topic) => (
                        <TopicItem 
                          ...
                          topic={topic} 
                          type="task2"
                          onStart={() => onStartTopic(topic.id, 'task2')}
                        />
                      ))
                    )}
                  </div>
    ...
    <div className={`flex items-center justify-between p-3 rounded-lg ${
      ...
      <div className="flex items-center gap-3">
        <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-medium ${
          ...
          {topic.isCompleted ? <CheckCircle className="w-4 h-4" /> : topic.orderIndex}
        </div>
